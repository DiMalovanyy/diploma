...
\newtheorem{theorem}{Твердження}[section]

\begin{document}

	\input{title_page} 		% Титулка
 	\input{abstract}		% Реферат
 	\input{contents_page} 	% Зміст
    \input{legend}          % Скорочення та умовні позначення
 	
 	\input{intro} 			% Вступ
 
 	\input{chapter1}  		% Розділ 1
 	\input{chapter2} 		% Розділ 2
 	\input{results} 		% Висновки
 	\input{literature} 		% Перелік джерел
 	\input{appendix} 		% Додатки
